...
if __name__ == '__main__':
    ...
    max_lenss = max(token_lens)
    print("motion prompt lens" , opt.pre_lens)
    for r in range(opt.repeat_times):
        print("-->Repeat %d"%r)
        with torch.no_grad():

            mids = []
            for i ,(caption, token_len) in enumerate(zip(prompt_list, token_lens)):
                # import pdb
                # pdb.set_trace()
                temp_captions = []
                temp_captions.append(caption)
                temp_token_lens = token_len.unsqueeze(0)
                if i==0:
                    mids = t2m_transformer.generate(temp_captions, 
                                                    temp_token_lens,
                    ...
                    total_mids = torch.cat((total_mids, mid), dim=1)
                    if max_lenss - mid.size(1)>0:
                        padding_tensor = torch.full((1, max_lenss - mid.size(1)), -1,device=mid.device)
                        mid = torch.cat((mid, padding_tensor), dim=1)
                    mids = torch.cat((mids, mid), dim=0)
                # print(mids)
                # print(mids.shape)
            ...
            #==============
            total_mids.unsqueeze_(-1)
            total_pred_motions = vq_model.forward_decoder(total_mids)
            total_pred_motions = total_pred_motions.detach().cpu().numpy()
            total_data = inv_transform(total_pred_motions)
        # import pdb;pdb.set_trace()

        total_len = total_data.shape[1]
        k = 'all'
        print("---->Sample %d: %s %d"%(r, "all", total_len))
        ...
        joint_data = total_data[0]
        joint = recover_from_ric(torch.from_numpy(joint_data).float(), 22).numpy()

        bvh_path = pjoin(animation_path, "sample_all_repeat%d_len%d_ik.bvh"%(r, total_len))
        _, ik_joint = converter.convert(joint, filename=bvh_path, iterations=100)

        bvh_path = pjoin(animation_path, "sample_all_repeat%d_len%d.bvh"%(r, total_len))
        _, joint = converter.convert(joint, filename=bvh_path, iterations=100, foot_ik=False)


        save_path = pjoin(animation_path, "sample_all_repeat%d_len%d.mp4"%(r, total_len))
        ik_save_path = pjoin(animation_path, "sample_all_repeat%d_len%d_ik.mp4"%(r, total_len))

        plot_3d_motion(ik_save_path, kinematic_chain, ik_joint, title=caption, fps=20)
        plot_3d_motion(save_path, kinematic_chain, joint, title=caption, fps=20)
        np.save(pjoin(joint_path, "sample_all_repeat%d_len%d.npy"%(r, total_len)), joint)
        np.save(pjoin(joint_path, "sample_all_repeat%d_len%d_ik.npy"%(r, total_len)), ik_joint)
        ...
        # total_len = total_data.shape[1]
        # k = 'all'
        # print("---->Sample %d: %s %d"%(k+1, "all", total_len))
        # animation_path = pjoin(animation_dir, str(k))
        # joint_path = pjoin(joints_dir, str(k))
